console.log('node boot html start 1');
...
const vrMode = process.argv[5];
const vrTexture = parseInt(process.argv[6], 10);
const vrTexture2 = parseInt(process.argv[7], 10);
...
global.onDrawFrame = (viewMatrixFloat32Array, projectionMatrixFloat32Array, centerFloat32Array) => {
  // console.log('on draw frame', !!window);
  if (window) {
    ...
    new THREE.Matrix4().toArray(viewMatrixFloat32Array);
    
    // console.log('on draw frame', viewMatrixFloat32Array.join(','), projectionMatrixFloat32Array.join(','));
...
exokit.fetch(url, {
  ...
  .then(site => {
    ...
    window.navigator.setVRTextures([vrTexture, vrTexture2]);
